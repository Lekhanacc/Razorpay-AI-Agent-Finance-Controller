...
Pipeline (matches the architecture given for Phase 4):
  ...
  NLU predict -> evaluate_confidence (PROCEED / CLARIFY / OUT_OF_DOMAIN)
    ...
    -> CLARIFY outside the RAZORPAY domain (AMBIGUOUS or unrecognized): ask for
       clarification, no retrieval, no generation
    -> RAZORPAY domain (whether the decision is PROCEED, or CLARIFY on intent
       confidence alone): retrieve evidence (semantic primary, TF-IDF fallback)
         -> no evidence above threshold: return INSUFFICIENT_EVIDENCE (or
            CLARIFY, if the low-confidence decision was CLARIFY), no Gemini call
...
Intent confidence alone is deliberately not a hard gate once the domain
classifier confidently says RAZORPAY: the intent taxonomy is fine-grained
(21 classes trained on a modest dataset) and short natural questions such as
"What is a payment link?" routinely score below INTENT_CONFIDENCE_THRESHOLD
even though they are clearly in-scope and well covered by the corpus. Domain
confidence plus real retrieval evidence is treated as the trustworthy signal
for these; intent confidence still gates CLARIFY when no evidence is found.
